      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <ModeIndicator className="inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r from-blue-500 to-purple-500 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-white shadow-sm" />
          </div>
          <Badge variant={getStatusBadgeVariant()} className="capitalize text-sm px-3 py-1 shadow-sm">
            {state}
          </Badge>
        </div>
          ...
          <CardContent className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="rounded-lg border border-border p-2.5 bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-950 dark:to-blue-900">
                <p className="text-muted-foreground text-[10px] uppercase tracking-wider font-bold">Mode</p>
              ...
              <div className="rounded-lg border border-border p-2.5 bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-950 dark:to-purple-900">
                <p className="text-muted-foreground text-[10px] uppercase tracking-wider font-bold">Devices</p>
          ...
          <CardContent>
            <div className="grid grid-cols-3 gap-2">
              ...
              <Button
                ...
                variant="default"
                size="sm"
                className="text-xs h-9 font-semibold shadow-sm bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600"
              >
                Start
              </Button>
              <Button
      ...
      <Card className="shadow-md hover:shadow-lg transition-shadow">
        ...
        <CardContent>
          <div className="rounded-lg border border-border bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-950 dark:to-slate-900 p-3 font-mono text-[11px] max-h-32 overflow-auto space-y-1 shadow-inner">
            {log.length > 0 ? (
              log.map((line, idx) => (
                <div key={`${line}-${idx}`} className="text-muted-foreground hover:text-foreground transition-colors py-0.5 px-1 rounded hover:bg-background/50">
                  {line}
                </div>
              ))
            ) : (
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
            {Object.entries(flags)
              ...
              .map(([name]) => (
                <div key={name} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-gradient-to-r from-emerald-50 to-green-50 dark:from-emerald-950 dark:to-green-950 border border-emerald-200 dark:border-emerald-800 shadow-sm">
                  <div className="h-2 w-2 rounded-full bg-emerald-500 shadow-sm animate-pulse" />
                  <span className="capitalize text-xs font-semibold">{name.replace(/([A-Z])/g, ' $1')}</span>
                </div>
              ))}
          </div>
        </CardContent>
      ...
      <div className="col-span-1 md:col-span-2 lg:col-span-4">
        <CloudOnly>
          <Card className="border-2 border-blue-200 bg-gradient-to-br from-blue-50 to-blue-100 dark:border-blue-700 dark:from-blue-950 dark:to-blue-900 shadow-md hover:shadow-lg transition-all">
            <CardHeader className="pb-2 pt-4">
              <CardTitle className="text-blue-900 dark:text-blue-100 text-base font-bold">☁️ Cloud Mode</CardTitle>
            </CardHeader>
        ...
        <Feature name="adminDashboard">
          <Card className="border-2 border-emerald-200 bg-gradient-to-br from-emerald-50 to-emerald-100 dark:border-emerald-700 dark:from-emerald-950 dark:to-emerald-900 shadow-md hover:shadow-lg transition-all">
            ...
            <CardContent className="text-emerald-800 dark:text-emerald-100 text-xs">
              Admin features enabled.
            </CardContent>
          </Card>
        </Feature>
